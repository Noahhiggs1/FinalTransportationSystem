...
public class AdminAnalyticsController {
    ...
    public Map<String, Object> getRevenueSummary() {
        ...
        double totalRevenue = all.stream().filter(p -> "COMPLETED".equals(p.getStatus())).mapToDouble(p -> p.getAmount() != null ? p.getAmount() : 0).sum();
        double refundedAmount = all.stream().filter(p -> "REFUNDED".equals(p.getStatus())).mapToDouble(p -> p.getAmount() != null ? p.getAmount() : 0).sum();
        Map<String, Double> byMethod = all.stream().filter(p -> "COMPLETED".equals(p.getStatus())).collect(Collectors.groupingBy(p -> p.getMethod() != null ? p.getMethod() : "UNKNOWN", Collectors.summingDouble(p -> p.getAmount() != null ? p.getAmount() : 0)));
    ...
    public List<Refund> getPendingRefunds() {
        return refundRepository.findAll().stream().filter(r -> "PENDING".equals(r.getStatus())).collect(Collectors.toList());
    }
    ...
    public Map<String, Object> getTicketSummary() {
        ...
        Map<String, Long> byStatus = all.stream().collect(Collectors.groupingBy(t -> t.getBookingStatus() != null ? t.getBookingStatus() : "UNKNOWN", Collectors.counting()));
        Map<String, Object> summary = new LinkedHashMap<>();
    ...
    public Map<String, Object> getFleetSummary() {
        ...
        double avgOccupancy = totalCapacity > 0 ? (double) totalOccupied / totalCapacity * 100 : 0;
        Map<String, Object> summary = new LinkedHashMap<>();
    ...
    public List<CrowdLog> getAllCrowdLogs() { return crowdLogRepository.findAll(); }

    @GetMapping("/crowd/summary")
    public Map<String, Object> getCrowdSummary() {
        List<CrowdLog> all = crowdLogRepository.findAll();
        Map<String, Long> byStatus = all.stream().collect(Collectors.groupingBy(l -> l.getStatus() != null ? l.getStatus() : "UNKNOWN", Collectors.counting()));
        OptionalDouble avgOcc = all.stream().mapToInt(CrowdLog::getOccupancyPercentage).average();
        Map<String, Object> summary = new LinkedHashMap<>();
    ...
    public List<DelayEvent> getAllDelays() { return delayEventRepository.findAll(); }

    @GetMapping("/delays/summary")
    public Map<String, Object> getDelaySummary() {
        List<DelayEvent> all = delayEventRepository.findAll();
        Map<String, Long> bySeverity = all.stream().collect(Collectors.groupingBy(d -> d.getSeverity() != null ? d.getSeverity() : "UNKNOWN", Collectors.counting()));
        Map<String, Long> byStatus   = all.stream().collect(Collectors.groupingBy(d -> d.getStatus()   != null ? d.getStatus()   : "UNKNOWN", Collectors.counting()));
        OptionalDouble avgDelay = all.stream().mapToInt(DelayEvent::getEstimatedDelayMinutes).average();
        Map<String, Object> summary = new LinkedHashMap<>();
    ...
    public List<IncidentReport> getAllIncidents() { return incidentReportMongoRepository.findAll(); }

    @GetMapping("/incidents/summary")
    public Map<String, Object> getIncidentSummary() {
        List<IncidentReport> all = incidentReportMongoRepository.findAll();
        Map<String, Long> bySeverity = all.stream().collect(Collectors.groupingBy(r -> r.getSeverity() != null ? r.getSeverity() : "UNKNOWN", Collectors.counting()));
        Map<String, Long> byStatus   = all.stream().collect(Collectors.groupingBy(r -> r.getStatus()   != null ? r.getStatus()   : "UNKNOWN", Collectors.counting()));
        Map<String, Object> summary = new LinkedHashMap<>();
        summary.put("totalIncidents", all.size());
        summary.put("bySeverity", bySeverity);
        summary.put("byStatus", byStatus);
        return summary;
    }
    ...
    public List<PassengerPattern> getAllPassengerPatterns() { return passengerPatternRepository.findAll(); }
