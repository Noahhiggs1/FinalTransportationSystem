...
public class OperatorController {
    ...
    public ResponseEntity<?> resolveIncident(@PathVariable String id) {
        return incidentReportMongoRepository.findById(id)
            .map(inc -> { inc.setStatus("RESOLVED"); inc.setResolvedAt(new Date()); return ResponseEntity.ok(incidentReportMongoRepository.save(inc)); })
            .orElse(ResponseEntity.notFound().build());
    }
    @GetMapping("/delays/{employeeId}")
    public List<DelayEvent> getDelays(@PathVariable Long employeeId) {
        return delayEventRepository.findByEmployeeIdOrderByCreatedAtDesc(employeeId);
    }
    ...
    public List<WorkLog> getWorkLogs(@PathVariable Long employeeId) {
        return workLogRepository.findByEmployeeIdOrderByLogDateDesc(employeeId);
    }
    @PostMapping("/clockin/{employeeId}")
    public ResponseEntity<?> clockIn(@PathVariable Long employeeId) {
        Optional<WorkLog> active = workLogRepository.findByEmployeeIdAndClockOutIsNull(employeeId);
        if (active.isPresent()) return ResponseEntity.badRequest().body("Already clocked in.");
        WorkLog log = new WorkLog();
        log.setEmployeeId(employeeId); log.setClockIn(LocalDateTime.now()); log.setLogDate(java.time.LocalDate.now());
    ...
    public ResponseEntity<?> clockOut(@PathVariable Long employeeId) {
        Optional<WorkLog> active = workLogRepository.findByEmployeeIdAndClockOutIsNull(employeeId);
        if (active.isEmpty()) return ResponseEntity.badRequest().body("Not clocked in.");
        WorkLog log = active.get(); log.setClockOut(LocalDateTime.now());
        Duration d = Duration.between(log.getClockIn(), log.getClockOut());
